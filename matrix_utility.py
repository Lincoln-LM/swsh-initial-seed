"""Utility functions for GF(2) matrices"""
...
def generalized_inverse(matrix: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    """Compute the generalized inverse & nullbasis of a matrix"""

    _, inverse_form, rank, pivots = reduced_row_echelon_form(matrix)
    nullbasis, *_ = reduced_row_echelon_form(inverse_form[rank:])
